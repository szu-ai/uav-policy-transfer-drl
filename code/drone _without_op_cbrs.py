#!/usr/bin/env python3
# drone2.py: PPO + OSD + Fuzzy baseline WITHOUT OP-CBRS reward shaping.
# Use this file to generate the reviewer-requested PPO-without-OP-CBRS ablation.
...
def parse_args():
    parser = argparse.ArgumentParser(description="GPS-denied drone inspection in Isaac Sim. PPO baseline WITHOUT OP-CBRS shaping.")
    parser.add_argument("--headless", action="store_true", help="Run without GUI.")
    parser.add_argument("--mode", type=str, default="train", choices=["train", "random", "collect_uniform_speed", "build_op_cbrs_library", "train_e1_osd", "transfer_e2", "eval_baselines", "eval_transfer", "paper_pipeline", "train_e1_osd_no_opcbrs", "transfer_e2_no_opcbrs", "eval_transfer_no_opcbrs", "paper_pipeline_no_opcbrs"], help="Algorithmic mode. This drone2.py variant is for PPO without OP-CBRS; OP-CBRS library modes are retained only for compatibility/diagnostics.")
    ...
    parser.add_argument("--policy-name", type=str, default="PPO-OSD+Fuzzy-NoOPCBRS", help="Name written into metrics CSV for paper tables.")
    ...
    parser.add_argument("--enable-op-cbrs", action="store_true", default=False, help="Use OP-CBRS-style potential shaping. In drone2.py this is OFF by default; do not pass this flag for the PPO-without-OP-CBRS ablation.")
    parser.add_argument("--disable-op-cbrs", action="store_true", default=True, help="Disable OP-CBRS reward shaping but still log potential diagnostics. Default True for drone2.py.")
...
class NPPDroneGymEnv(gym.Env):
    ...
    def __init__(
        ...
        self.enable_op_cbrs = bool(enable_op_cbrs)  # drone2.py baseline: False unless explicitly overridden
    ...
    def step(self, action):
        ...
        # Robust scalar-speed safety shield for Sim2Sim transfer.  The learned
        # PPO action remains the primary adaptive component, but it is blended
        # with the analytic fuzzy OSD speed and protected by a risk-aware floor.
        # This prevents the common failure mode where a transferred scalar-speed
        # policy becomes too conservative and cannot finish all waypoints.
        risk = self._clip01(
            0.45 * (1.0 - mu_T)
            + 0.25 * (1.0 - mu_L)
            + 0.20 * (1.0 - mu_W)
        ...
        # Keep a small lateral parallax motion in low-texture zones to help VSLAM,
        # but do not expose this as a separate policy action.
        if mu_T < 0.35 and dist_xy > 1.5:
...
def main():
    args = parse_args()
    # drone2.py is the PPO-without-OP-CBRS ablation. Force shaping OFF unless the user intentionally edits this file.
    args.enable_op_cbrs = False
    args.disable_op_cbrs = True
    ...
    def _write_algorithm_manifest(stage_name: str, extra: Optional[Dict[str, object]] = None):
        ...
        manifest = {
            "stage": stage_name,
            "methodology": "MDP + PPO-learned OSD + fuzzy inference + Sim2Sim transfer + VSLAM localization; OP-CBRS shaping disabled for ablation",
            ...
            "mdp": {
                "state": "SLAM pose/velocity/yaw, IMU-like motion, target-relative geometry, camera/feature cues, depth rays, tracking quality, previous scalar OSD action, mission progress",
                "action": "scalar OSD speed in A=[v_min,v_max]; the low-level controller tracks waypoint direction, yaw and altitude",
                "reward": "progress + waypoint + safety + fuzzy/adherence terms; OP-CBRS potential shaping disabled",
            },
    ...
    if args.mode == "random":
        ...
        obs = vec.reset()
        print("[INSPECT] Analytic OSD+Fuzzy inspection controller active (OP-CBRS disabled in drone2.py).")
    ...
    if args.mode in ["train_e1_osd", "train_e1_osd_no_opcbrs"]:
        train_ppo_policy("PPO-OSD+Fuzzy-NoOPCBRS-Source-e1", "osd_fuzzy_no_opcbrs_source_e1", "e1")
        simulation_app.close()
        return

    if args.mode in ["transfer_e2", "transfer_e2_no_opcbrs"]:
        source = args.source_model or str(Path(args.output_root).expanduser() / "models" / "osd_fuzzy_no_opcbrs_source_e1.zip")
        train_ppo_policy("PPO-OSD+Fuzzy-NoOPCBRS-Sim2Sim-Transfer-e2", "osd_fuzzy_no_opcbrs_transfer_e2", "e2", load_path=source, learn_steps=args.transfer_timesteps)
        simulation_app.close()
        return
    ...
    if args.mode in ["eval_transfer", "eval_transfer_no_opcbrs"]:
        model_path = args.transfer_model or args.source_model or str(Path(args.output_root).expanduser() / "models" / "osd_fuzzy_no_opcbrs_transfer_e2.zip")
        evaluate_model(model_path, f"Eval-Transferred-OSD+Fuzzy-NoOPCBRS-{args.sim_env_id}", args.sim_env_id, args.eval_episodes)
        simulation_app.close()
        return

    if args.mode in ["paper_pipeline", "paper_pipeline_no_opcbrs"]:
        # PPO-without-OP-CBRS ablation pipeline: do not collect or use the
        # OP-CBRS potential library. Potential diagnostics may still be logged
        # by the environment, but shaping is disabled by default in drone2.py.
        args.enable_op_cbrs = False
        args.disable_op_cbrs = True
        source = train_ppo_policy("PPO-OSD+Fuzzy-NoOPCBRS-Source-e1", "osd_fuzzy_no_opcbrs_source_e1", "e1")
        transfer = train_ppo_policy("PPO-OSD+Fuzzy-NoOPCBRS-Sim2Sim-Transfer-e2", "osd_fuzzy_no_opcbrs_transfer_e2", "e2", load_path=source, learn_steps=args.transfer_timesteps)
        eval_baselines()
        evaluate_model(transfer, "Eval-Transferred-OSD+Fuzzy-NoOPCBRS-e2", "e2", args.eval_episodes)
        _write_algorithm_manifest(
            "paper_pipeline_no_opcbrs",
            {
                "source_model": str(source),
                "transfer_model": str(transfer),
                "eval_episodes": int(args.eval_episodes),
                "op_cbrs_enabled": False,
            },
        )
        simulation_app.close()
        return
